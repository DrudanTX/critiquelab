import { Moon, Sun, Focus, RotateCcw } from "lucide-react";
...
export function LofiToggle() {
  ...
  const { focusMode, toggleFocusMode, ogMode, toggleOgMode } = useLofiMode();
      ...
      <Tooltip delayDuration={300}>
        <TooltipTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            onClick={toggleOgMode}
            className={`h-9 w-9 transition-all duration-slow ${
              ogMode
                ? "bg-accent/10 text-accent"
                : "text-muted-foreground hover:text-foreground"
            }`}
            aria-label="Toggle OG mode"
          >
            <AnimatePresence mode="wait">
              <motion.div
                key={ogMode ? "og-on" : "og-off"}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.8 }}
                transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }}
              >
                <RotateCcw className={`h-4 w-4 ${ogMode ? "fill-accent/20" : ""}`} />
              </motion.div>
            </AnimatePresence>
          </Button>
        </TooltipTrigger>
        <TooltipContent side="bottom" className="text-xs">
          <p>{ogMode ? "Switch to lo-fi mode" : "Switch to OG mode"}</p>
        </TooltipContent>
      </Tooltip>
